	...
	<header class="header">
		...
		<div class="header_content d-flex flex-row align-items-center justify-content-start">
			...
			<div class="header_right d-flex flex-row align-items-center justify-content-start ml-auto">
				...
				<div class="log_reg">
					<ul class="d-flex flex-row align-items-center justify-content-start">
						<li><a href="../../../DBMS_PROJECT/Login_v16/Login_v16/login.html">Login</a></li>
						<li><a href="../../../DBMS_PROJECT/colorlib-regform-17/Reg.html">Register</a></li>
					</ul>
				</div>
				<div class="hamburger">
			...
			<div class="container">
				<div class="row">
					...
					try{
						...
						$stmt1 = "CREATE VIEW student_genres AS (SELECT count(*) AS counta,genre FROM student,Borrows,book WHERE Email='".$_POST["email"]."' AND password='".$_POST["password"]."' AND Usn=usn_borrows AND Book_id_borrows=Book_id AND type='Fiction' GROUP BY genre)";
						$stmt1 = $conn->query($stmt1);
						$stmt2="SELECT DISTINCT book_name FROM book WHERE genre=(SELECT genre FROM student_genres WHERE counta>=ALL(SELECT counta FROM student_genres))";
						$stmt2 = $conn->query($stmt2);
						//$stmt="SELECT book_name FROM book WHERE genre='Fantasy'";
						//$stmt=$conn->query($stmt);
						$no = 1;
						while($result = $stmt2-> fetch(\PDO::FETCH_ASSOC) and $no <= 5) {
			...
			<div class="container">
				<div class="row">
					...
					try{
						...
						//querying database for table values
						$stmt = "SELECT DISTINCT book_name FROM student,book WHERE Email='".$_POST["email"]."' AND password='".$_POST["password"]."'
								 AND ((Did_Book IS NOT NULL AND Did_Book=Did_stud AND book.semester=student.semester) OR (Did_Book IS NULL AND book.semester=student.semester))"; 
						$stmt = $conn->query($stmt);
						$no = 1;
			...
			<div class="container">
				<div class="row">
					...
					try{
						...
						$stmt = $conn->query($stmt);
						$stmt1 = "SELECT book_name,overdue_fees FROM borrows,book,student WHERE overdue_fees>0 AND book_id=Book_id_borrows AND usn=usn_borrows AND Email='".$_POST["email"]."' AND password='".$_POST["password"]."'";
						$stmt1 = $conn->query($stmt1);
						$stmt2= "SELECT SUM(overdue_fees) AS total FROM borrows,book,student WHERE overdue_fees>0 AND book_id=Book_id_borrows AND usn=usn_borrows AND Email='".$_POST["email"]."' AND password='".$_POST["password"]."'";
						$stmt2 = $conn->query($stmt2);
						//$no = 1;
						while($result = $stmt1-> fetch(\PDO::FETCH_ASSOC)) {
								echo "<div class=\"location\">
								<img src=\"images/location_".$no.".jpg\" alt=\"\">
								<div class=\"location_title text-center\"><div><a href=\"#\">".$result['book_name']."(Rs.".$result['overdue_fees'].")<div>+</div></a></div></div>
								</div>";  
								//$no = $no +1;      
						}
