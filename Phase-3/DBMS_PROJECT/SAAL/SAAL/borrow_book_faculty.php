			...
			<div class="container">
				<div class="row">
					...
					try{
						...
						$stmt= "SELECT book_name,book_id FROM book,faculty WHERE Email='".$_COOKIE["Email"]."' AND password='".$_COOKIE["Password"]."' AND branch_id_fac=branch_id_book AND book_id NOT IN (SELECT book_id_borrows FROM BORROWS)";
...
<?php
		$host = "localhost";
					...
					try{
						...
						//$stmt1 = $conn->query($stmt1);
						$stmt1 = "SELECT * FROM book,faculty WHERE book_name='".$_POST["book"]."' AND Email='".$_COOKIE["Email"]."' AND password='".$_COOKIE["Password"]."' AND branch_id_book=branch_id_fac AND book_id NOT IN (SELECT book_id_borrows FROM BORROWS)";
						$stmt1 = $conn->query($stmt1);
						$result = $stmt1-> fetch(\PDO::FETCH_ASSOC); 
						$stmt= "SELECT MAX(borrowal_id)+1 AS maxa FROM borrows";
						$stmt = $conn->query($stmt);
						$result2 = $stmt-> fetch(\PDO::FETCH_ASSOC); 
						$stmt2 = "INSERT INTO borrows VALUES('".$result2["maxa"]."',current_date::date+30,NULL,'".$result["fid"]."','".$result["book_id"]."',0)";
						$stmt2 = $conn->query($stmt2);
