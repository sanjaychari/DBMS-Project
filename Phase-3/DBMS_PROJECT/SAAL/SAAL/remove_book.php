			...
			<div class="container">
				<div class="row">
					<div class="col">
						...
						<div class="section_title text-center"><h1>Un-Borrowed Books In Your Branch</h1></div>
					...
					try{
						...
						$stmt= "SELECT book_name,book_id FROM book,admin WHERE Email='".$_COOKIE["Email"]."' AND password='".$_COOKIE["Password"]."' AND branch_id_admin=branch_id_book AND book_id NOT IN (SELECT book_id_borrows FROM BORROWS)";
						...
						<div class="listings_content">
							...
							<div class="listing_search_container">
									...
									<div class="d-flex flex-md-row flex-column align-items-start justify-content-start">
										<div>
											<input type="text" class="search_input" placeholder="Enter the name of the book you want to remove." required="required" name="book"></div><div>
											<button class="search_button" type="submit" name="submit">Remove</button>
										</div>
									</div>
									</form>
							</div>
...
<?php
		$host = "localhost";
					...
					try{
						...
						//$stmt1 = $conn->query($stmt1);
						$stmt1 = "DELETE FROM BOOK WHERE book_id=(SELECT book_id FROM book,admin WHERE book_name='".$_POST["book"]."' AND Email='".$_COOKIE["Email"]."' AND password='".$_COOKIE["Password"]."' AND branch_id_admin=branch_id_book AND is_borrowed=FALSE)";
						$stmt1 = $conn->query($stmt1);
						//$stmt2 = "DELETE FROM book WHERE book_id_borrows IN (SELECT book_id FROM book,student,borrows WHERE book_name='".$_POST["book"]."' AND Email='".$_COOKIE["Email"]."' AND password='".$_COOKIE["Password"]."' AND book_id_borrows=book_id AND usn_borrows=usn)"; 
						//$stmt2 = $conn->query($stmt2);
						//$stmt2="DROP VIEW yourbooks";
						//$stmt3 = $conn->query($stmt3);
						//	$stmt4 = "IF (SELECT book_id FROM yourbooks) NOT IN book THEN INSERT INTO book VA"
						echo "<br>REMOVAL SUCCESSFUL<br>";}
					}
					catch (PDOException $e){
					// report error message
					echo $e->getMessage();
