...
$password = "mypass";
 ...
 switch($_POST['option']) {
		...
		// set the PDO error mode to exception
			...
			$stmt = "SELECT book_name,pname,book_id FROM book,publisher WHERE book_name LIKE '%".$_POST['query']."%' AND pid_book=pid"; 
	...
	$conn = null;
		...
		// set the PDO error mode to exception
			...
			//querying database for table values
			$stmt = "SELECT * FROM book,author WHERE fname LIKE '%".$_POST['query']."%' OR lname LIKE '%".$_POST['query']."%' AND au_id=Au_id_book"; 
			$stmt1 = $conn->query($stmt);
			$row = $stmt1->fetch(PDO::FETCH_ASSOC);
  			print "<br> <tr> ";
  			foreach ($row as $field => $value){
	...
	$conn = null;
		...
		// set the PDO error mode to exception
			...
			//querying database for table values
			$stmt = "SELECT * FROM book,publisher WHERE pname LIKE '%".$_POST['query']."%' AND pid=pid_book"; 
			$stmt1 = $conn->query($stmt);
			$row = $stmt1->fetch(PDO::FETCH_ASSOC);
  			print " <br><tr> ";
  			foreach ($row as $field => $value){
	...
	$conn = null;
		...
		// set the PDO error mode to exception
			...
			//querying database for table values
			$stmt = "SELECT * FROM book WHERE genre LIKE '%".$_POST['query']."%'"; 
			$stmt1 = $conn->query($stmt);
			$row = $stmt1->fetch(PDO::FETCH_ASSOC);
  			print "<br> <tr> ";
  			foreach ($row as $field => $value){
